...
- [Chapter 2　StanとRStanをはじめよう](#chapter-2stanとrstanをはじめよう)
  - [2.1　StanとRStanの準備](#21stanとrstanの準備)
  - [2.2　Stanの基本的な文法](#22stanの基本的な文法)
  - [2.3　補足と文献案内](#23補足と文献案内)
- [Chapter 3　基本的な回帰とモデルのチェック](#chapter-3基本的な回帰とモデルのチェック)
  - [3.1　回帰モデルを読むための共通枠組み](#31回帰モデルを読むための共通枠組み)
  - [3.2　単回帰](#32単回帰)
  - [3.3　重回帰](#33重回帰)
  - [3.4　二項ロジスティック回帰](#34二項ロジスティック回帰)
  - [3.5　ロジスティック回帰](#35ロジスティック回帰)
  - [3.6　ポアソン回帰](#36ポアソン回帰)
  - [3.7　モデルチェックのまとめ](#37モデルチェックのまとめ)
  - [3.8　補足と文献案内](#38補足と文献案内)
...
### 2.2.5　`generated quantities`ブロック
...
|---|---|---|---|---|
| 3.2 | 単回帰 | 週単位・地域単位 | 下水中ウイルス濃度と症例率 | 傾き、予測値、しきい値超過確率 |
| 3.3 | 重回帰 | 週単位・地域単位 | 入院率と症例数、接種率、移動量 | 調整済み係数、シナリオ予測 |
| 3.4 | 二項ロジスティック回帰 | 集計データ | 検査陽性数 / 検査数 | 陽性確率、オッズ比 |
| 3.5 | ロジスティック回帰 | 個人単位 | 感染有無、曝露、接種歴 | 個人リスク、オッズ比、VE風指標 |
| 3.6 | ポアソン回帰 | 地域・週のカウント | 症例数、人口オフセット | 発生率比、予測症例数 |

---

## 3.1　回帰モデルを読むための共通枠組み
...
3.4は週単位の集計データでした。ここでは、個人単位の感染有無を扱います。たとえば、接触者調査、症例対照研究、検査陰性デザイン、血清疫学調査では、個人ごとに感染あり/なし、ワクチン接種あり/なし、曝露あり/なし、年齢、マスク着用などを持ちます。
